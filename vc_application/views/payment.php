  ...
  <div class="modal-dialog"> 
    ...
    <div class="modal-content">
	  ...
	  <?php } ?>
		...
		OTP sent to ending number ******<?php echo substr($this->session->userdata('p_phone'),-4);?>
...
<div class="clearfix"></div>
	...
	<div class="setup-content" id="step-2">
	 <div class="col-md-4 col-xs-12 to-0-1">
	...
	<h4 class="cft">How do you want to pay ?</h4>
          <div class="you-pay">
        <ul>
		<!-- <li><label><input type="radio" <?php //if($this->input->post('how_to_pay') == 'ccavenue' || $this->input->post('how_to_pay') != 'blissperks') { echo 'checked="checked"'; } ?> name="how_to_pay" value="ccavenue">Pay Online</label></li> 
		
		<li class=""><label class="<?php //if($this->session->userdata('no_veryfied') != 'yes') { echo 'codradio'; } ?>"><input type="radio" <?php //if($this->input->post('how_to_pay') == 'cod') { echo 'checked="checked"'; } ?> name="how_to_pay" value="cod"> Cash On Delivery</label></li>  -->
		
		<?php 
		$cust_id = $this->session->userdata('cust_id');
		//if($cust_id!='') { 
          //if(isset($profile) && $profile[0]['bliss_amount'] >= $this->session->userdata('order_total')) {
			  echo '<li><label class="blissperkradio"><input type="radio" checked ';
			  //if($this->input->post('how_to_pay') == 'blissperks') { echo ' checked="checked" '; }
			  echo ' name="how_to_pay" value="eWallet">Available balance in eWallet <div class="kkk">'.$profile[0]['bliss_amount'].'</div></label><br>
			  
			  <a href="https://www.stakein.co.in/admin/request-wallet" type="button" class="btn btn-success btn-sm bt Combo">Load Wallet</a> 

			  <span style="font-size:12px;display:block;">Coupon will not apply at eWallet</span></li> ';
		  //}
	//	}
		?>
...
<div class="col-md-4 col-xs-12 to-0-1">
		...
		<button type="submit" class="btn btn-primary add-asrd" >Submit</button>
		</div>
		</div>
