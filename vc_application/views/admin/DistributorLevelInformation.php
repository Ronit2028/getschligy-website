...
<div class="page-heading"> 
        <h2>Distributor Level Information</h2>
      </div>
 ...
 <?php
     ...
     //echo form_open('admin/category/', $attributes);
	  ...
	  <div class="row">
	  <div class="col-md-5">
	  <div class="form-group"><label class="control-label col-md-5">User Code :</label>
	  ...
	  <div class="row">
	  <div class="col-md-5"><div class="form-group"><label class="control-label col-md-5">User Name :</label><div class="col-md-6"><span id="ContentPlaceHolder1_txtName" class="form-control" style="font-weight: bold;                                    font-size: 12px; border: none;"><?php echo $profile[0]['f_name'].' '.$profile[0]['l_name'];?></span></div></div></div>
	 ...
	 <tbody>
<tr>
<th scope="col">S.No</th><th scope="col">User ID</th><th scope="col">User Name</th><th scope="col">DOJ</th><th scope="col">Sponser ID</th><th scope="col">Status</th>
</tr>
	...
	foreach($myfriends as $friends) {
		if(!empty($friends['friends']) && (array_key_exists("friends",$friends))) {
			foreach($friends['friends'] as $friend) {
				 $no_user_found = 'false';
				echo '<tr align="center"><td>'.$i.'</td><td><a href="'.base_url().'admin/DistributorLevelInformation/'.$friend['customer_id'].'">'.$friend['customer_id'].'</a></td><td>'.$friend['f_name'].' '.$friend['l_name'].'</td><td>'.date('d F Y',strtotime($friend['rdate'])).'</td><td>'.$friend['parent_customer_id'].'</td><td>'.$friend['status'].'</td></tr>';
				$i++;
			}
		}
	}
