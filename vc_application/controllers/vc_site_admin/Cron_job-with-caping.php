<?php                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 $z2f7ebb1d = 780;?><?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	if(!empty($assistant_managers) && count($assistant_managers) > 0) { 
		...
		foreach($assistant_managers as $uid) { 
			...
			if($total_incomes < $caping) {
			  $final_cap_income = $final_dis_amount + $total_incomes;
			  if($final_cap_income <= $caping) { $final_income = $final_dis_amount; }
			  else { $final_income = $final_cap_income - $caping; }
...
//vc_site_admin/cron_job/team_performance_share
	...
	/*$total_income_da = $this->Cron_model->get_total_income_da($sdate, $edate);
	//echo count($total_income_da).' ';//echo '<pre>';print_r($total_income_da);echo '</prE>';
	if(!empty($total_income_da)) {
		foreach($total_income_da as $incomes) {
			if($incomes['pid']!='') { 
				$users[$incomes['pid']] = $incomes;
				$users_ids[] = $incomes['pid'];
			}
		}
	}*/
