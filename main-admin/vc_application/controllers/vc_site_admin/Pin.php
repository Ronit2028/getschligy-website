			...
			if ($this->input->server('REQUEST_METHOD') === 'POST') {
				...
				/*elseif(empty($user_ids)){
					...
					foreach($payouts as $con){ 
						...
						$monthly_bill = $this->coupon_model->get_monthly_bill($sdate,$con['user_id']);
						$monthly_pinbill = $this->coupon_model->get_monthly_pinbill($sdate,$con['user_id']);

						if($monthly_bill[0]['total_order']+$monthly_pinbill[0]['total_order']+0 >= 1000) {
							$tr_log = array('user_id'=>$con['user_id'],'amount'=>$total_amount,'type'=>'Weekly closing','status'=>'Weekly Closing');
							$this->coupon_model->add_transactional_log($tr_log);
							$this->coupon_model->substract_wallet($con['user_id'],$total_amount,'income_wallet');
						}
						else {
							$tr_log = array('user_id'=>$con['user_id'],'amount'=>$total_amount,'type'=>'Weekly closing','status'=>'Flash');
							$this->coupon_model->add_transactional_log($tr_log);
						}
					}
					if(!empty($all_users)) {
					    $this->coupon_model->update_income_status_all_user($all_users,$week_end);
					}
