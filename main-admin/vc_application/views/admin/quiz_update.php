	<style>
	 ...
	 //form data
      ...
      echo form_open_multipart('admin/quiz/edit/'.$this->uri->segment(4).'', $attributes);
	  $prod = $product[0];
	  ?>
      <div class="page-heading"> 
        <h2 class="iod">Update quiz <ul class="list-inline"><li><a class="btn btn-primary btn-sm" href="<?php echo base_url().'admin/quiz'; ?>">&laquo; Back</a></li><li><button type="submit" class="btn btn-primary btn-sm">Save</button></li><li><button type="submit" class="btn btn-primary btn-sm">Save & Continue</button></li></h2>
      ...
      ?>
		...
		<div class="form-group col-sm-12">
          <label  class="control-label col-sm-3">Enter Quiz title * <small style="color:red">*</small></label>
             <div class="col-sm-9"> <input type="text" class="form-control" required name="name" value="<?php if($this->input->post('name')!='') { echo $this->input->post('name'); } else { echo $prod['title']; }  ?>" >
          </div>
          </div>
		 ...
		 <div class="form-group col-sm-12">
          <label  class="control-label col-sm-3">Enter total number of questions <small style="color:red">*</small></label>
             <div class="col-sm-9"> <input type="text" class="form-control" required name="question" value="<?php if($this->input->post('question')!='') { echo $this->input->post('question'); } else { echo $prod['total']; }  ?>" >
          </div>
          </div>
		  ...
		  <div class="form-group col-sm-12">
           <label  class="control-label col-sm-3">Enter marks on right answer <small style="color:red">*</small></label>
             <div class="col-sm-9">
                <input type="text" class="form-control" required name="sahi" value="<?php if($this->input->post('sahi')!='') { echo $this->input->post('sahi'); } else { echo $prod['sahi']; }  ?>" > 
          </div> 
          </div> 
