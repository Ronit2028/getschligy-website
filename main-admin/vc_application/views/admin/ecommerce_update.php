	 ...
	 //form data
      ...
      echo form_open_multipart('admin/ecommerce/edit/'.$this->uri->segment(4).'', $attributes);
	  $prod = $product[0];
	  ?>
      <div class="page-heading"> 
        <h2 class="iod">Update product <ul class="list-inline"><li><a class="btn btn-primary btn-sm" href="<?php echo base_url().'admin/ecommerce'; ?>">&laquo; Back</a></li><li><button type="submit" class="btn btn-primary btn-sm">Save</button></li><li><button type="submit" class="btn btn-primary btn-sm">Save & Continue</button></li><li><button class="btn btn-primary btn-sm show-attributes" type="button">Creat New Attribute</button></li></h2>
      ...
      ?>
        ...
        <label  class="control-label col-sm-3">Stock keeping unit(SKU) <small style="color:red">*</small></label>
		  ...
		  <div class="form-group col-sm-12">
           <label  class="control-label col-sm-3">Combo Pack <small style="color:red">*</small></label>
             <div class="col-sm-9">  <select name="combo" class="form-control custom-select">
			  <option selected="" value="0">No</option>
			  <option <?php if($prod['combo']=='1') { echo 'selected="selected"'; } ?> value="1">Yes</option>
			  </select>
          </div> 
          </div> 
           ...
           <label  class="control-label col-sm-3">Category<small style="color:red">*</small></label>
			...
			<option value="">Select</option> 
			  ...
			  <option <?php if($prod['category']==1) { echo ' selected="selected" '; } ?> value="1">Combo Pack</option> 
			  </select>
          </div>  
          </div>
...
<div class="col-sm-12">	 
		...
		<h2></h2>
		  ...
		  <div class="form-group col-lg-6 col-mg-6 col-sm-6">
            <label  class="control-label col-sm-5">Procurement Cost</label>
             <div class="col-sm-7"> <input type="number" required="required"  step="0.01"  class="form-control"  name="cost" value="<?php if($this->input->post('cost')!='') { echo $this->input->post('cost'); } else { echo $prod['cost']; }  ?>" >
          </div> 
          </div> 
		   ...
		   <div class="form-group col-lg-6 col-mg-6 col-sm-6">
            <label  class="control-label col-sm-5">Selling Price</label>
             <div class="col-sm-7"> <input type="number" class="form-control"  step="0.01"  name="p_d_price" value="<?php if($this->input->post('p_d_price')!='') { echo $this->input->post('p_d_price'); } else { echo $prod['p_d_price']; }  ?>" >
          </div> 
          </div> 
			
			<div class="form-group col-lg-6 col-mg-6 col-sm-6">
            <label  class="control-label col-sm-5">PV</label>
             <div class="col-sm-7"> <input type="text" required="required"  step="0.01"  class="form-control"  name="comm_dis" value="<?php if($this->input->post('comm_dis')!='') { echo $this->input->post('comm_dis'); } else { echo $prod['comm_dis']; }  ?>" >
          </div> 
          </div>
		  ...
		  <div class="form-group col-lg-6 col-mg-6 col-sm-6">
            <label  class="control-label col-sm-5">BV</label>
             <div class="col-sm-7"> <input type="text"  class="form-control"  step="0.01"  name="bv" value="<?php if($this->input->post('bv')!='') { echo $this->input->post('bv'); } else { echo $prod['bv']; }  ?>" >
          </div> 
          </div>
          ...
          <label  class="control-label col-sm-5">Special price to Date</label>
             <div class="col-sm-7"> <input type="text" id="datepicker3" class="form-control"  name="sptdate" value="<?php if($this->input->post('sptdate')!='') { echo $this->input->post('sptdate'); } else { echo $prod['sptdate']; }  ?>" >
          </div>
          </div>
  ...
  <div class="form-group col-sm-12">
            <label  class="control-label">Description</label>
               <textarea class="form-control myTextarea2" name="p_discription"><?php if($this->input->post('p_discription')!='') { echo $this->input->post('p_discription'); } else { echo $prod['description']; } ?></textarea>
          </div>
		...
		<input type="hidden" value="<?php echo $prod['image'];?>" name="image_old">
         ...
         <?php if($prod['images']!=''){
				...
				foreach($imagesArray as $imagesVal) {
					echo '<div class="remove-i-div-'.$count.'"><div class="form-group col-sm-3"><input type="hidden" value="'.$imagesVal.'" name="images_old[]"><img class="img-responsive" src="'.base_url().'images/product/'.$imagesVal.'" ><button data-img="'.$imagesVal.'" data=".remove-i-div-'.$count.'" type="button" class="remove-btn glyphicon glyphicon-remove remove-image-div"></button></div></div>';
					$count++;
				}
			} ?>	
