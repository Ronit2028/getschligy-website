	...
	width: 100%;
	 ...
	 //form data
      ...
      echo form_open_multipart('admin/myschlorship/edit/'.$this->uri->segment(4).'', $attributes);
	  $prod = $schlorship_id[0];
	  ?>
      <div class="page-heading"> 
        <h2 class="iod">Scholarship Update<ul class="list-inline"><li><a class="btn btn-primary btn-sm" href="<?php echo base_url().'admin/myschlorship'; ?>">&laquo; Back</a></li><li><button type="submit" class="btn btn-primary btn-sm">Save</button></li><li><button type="submit" class="btn btn-primary btn-sm">Save & Continue</button></li></h2>
      ...
      if($this->session->flashdata('flash_message')){
        if($this->session->flashdata('flash_message') == 'updated')
        {
          echo '<div class="alert alert-success">';
            ...
            echo '<strong>Well done!</strong> Schlorship Scheme updated successfully.';
          echo '</div>';       
      ...
      ?>
		...
		<input type="hidden" value="<?php echo $prod['image'];?>" name="image_old">
               ...
               <div class="col-sm-7"><?php if($prod['image']!='') { echo '<img width="150" class="img-responsive" src="'.base_url().'images/'.$prod['image'].'" >'; } ?> </div>
		  ...
		  <div class="form-group col-sm-12">
           <label  class="control-label col-sm-3">Type <small style="color:red">*</small></label>
             <div class="col-sm-9">  <select name="type" class="form-control custom-select">
			  ...
			  <option <?php if($prod['type']=='Latest') { echo 'selected="selected"'; } ?> value="Latest">Latest</option>
			  <option <?php if($prod['type']=='Others') { echo 'selected="selected"'; } ?> value="Others">Others</option>
			  </select>
          </div> 
          </div> 
		...
		<!--  <div class="form-group col-sm-12">
            <label  class="control-label">Short Description</label>
              <textarea class="form-control myTextarea2" name="s_discription"></textarea>
          </div>-->
		  
  <div class="form-group col-sm-12">
            <label  class="control-label">Description</label>
               <textarea class="form-control myTextarea2" name="discription"><?php if($this->input->post('discription')!='') { echo $this->input->post('discription'); } else { echo $prod['discription']; } ?></textarea>
          </div>
...
<!-- Description end-->
		...
		<!--   <div class="form-group col-sm-12">
            <label  class="control-label">Discription</label>
               <textarea class="form-control textarea-editor" name="discription"><?php if($this->input->post('discription')!='') { echo $this->input->post('discription'); } else { echo $prod['discription']; } ?></textarea>
          </div>   -->
