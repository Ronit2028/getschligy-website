...
<table  class="table table-bordered table-hover customer-table"> 
	<thead> <tr><th>ID</th> <th>Customer ID</th><th>Email</th><th>Phone</th><th>Subject</th><th>Message</th><th>Reply</th><th>Type</th><th>Rating</th><!--th>Earnings</th--><th>status</th><!--th>Delete</th--> </tr> </thead> 
<tbody> 
<?php 
$i = 1;
foreach($customer as $con){ 
	if($con['status'] == 'accepted') { $st='act'; } elseif($con['status'] == 'rejected') {  $st='rej'; } else { $st=''; }
	echo '<tr class='.$st.'><td>'.$i.'</td><td><a href="'.base_url().'admin/feedback/edit/'.$con['id'].'">'.$con['name'].'</a></td><td>'.$con['email'].'</a></td><td>'.$con['phone'].'</td><td>'.$con['subject'].'</td><td>'.substr($con['message'],0,30).'</td><td>'.substr($con['reply'],0,30).'</td><td>'.$con['type'].'</td><td>'.$con['rating'].'</td><td>'.$con['status'].'</td>';
